...
public class ResourceWorkQueueTest {
	...
	public void testManipulateResourceQueue() {
		...
		myQueueRef.addUOW("testResourceId3",
				"This is the task to add for processing");
		...
		assertEquals("This is the task to add for processing",
				myQueueRef.removeUOW("testResourceId3"));

		// we'll do them out of sequence to test they are in fact two separate
		// queues
		assertEquals("This one only gets one task",
				myQueueRef.removeUOW("testResourceId4"));

		assertEquals("And a second one",
				myQueueRef.removeUOW("testResourceId3"));
	}
