...
public class FixtureActionProcessorTest {
	...
	public void testResourceImplCalled() {
		/*
		 * This mocks the side effect ResourceImpl has on the it's work queue
		 * (i.e. it removes it's work). You can prove to yourself the work queue
		 * is populated by looking at the logs for
		 * "FixtureActionProcessor - Processing started for fixture/resource:"
		 */
		...
		/*
		 * We're checking that fixtureActionProcessor has called ResourceImpl to
		 * dequeue the task (and process it), but we're not testing that here,
		 * we're only testing that fixtureActionProcessor calls it
		 */
