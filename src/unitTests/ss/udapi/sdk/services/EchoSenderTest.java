...
public class EchoSenderTest {
	...
	public void setUp() throws Exception {
		...
		echoSender = EchoSender
				.getEchoSender(
						"http://xxx.test123url.com/UnifiedDataAPI/stream/echo/UJp-nkCrpBHv195n1Oi2rWm9TCox",
						resRequest);
	...
	public void testEchoRetryNotExceeded() {
		...
		/*
		 * We are checking to see if the echo count achieved the maximum number
		 * the count appears to be a plus one but that's becauseof where we
		 * carry out the check in echo sender (ie when we next call it).
		 */
		assertTrue(EchoResourceMap.getEchoMap().getEchoCount("testresource2")
				.equals(0));
