...
public class EchoResourceMap {
	...
	/*
	 * Not currently used, but seemed sensible to include such a method as an
	...
	protected void resetEchoCount(String resourceId) {
		if (resourceId != null) {
			...
			logger.info("Echo or message received for fixture Id: "
					+ resourceId + ". Current missed echos: "
					+ map.get(resourceId));
		}
	}

	/*
	 * Not overly tidy, but in order to avoid looping twice through this set
	 * (once here and once in EchoSender we prepare the list of defaulters here.
	 * Could be moved to EchoSender and it would still work, just means
	 * iterating through this set twice.
	 * 
	 * Defaulters is a list of all resources that have reached the maximum
	 * number of echo response failures.
	 */
	protected Set<String> incrAll(int retries) {
		...
		while (keyIter.hasNext()) {
			...
			logger.debug("Echo count increased for: " + resourceId
					+ ". Current count " + (map.get(resourceId)));
		}
		return defaulters;
	}
