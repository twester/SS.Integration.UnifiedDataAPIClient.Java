...
public class MQListener implements Runnable {
	...
	public synchronized static MQListener getMQListener(String amqpDest) {
		 ...
		 * lead to a corrupt object without synchronization, which in turn
	...
	public void run() {
		terminate = false;
		/*
		 * This section happens only once when the thread is kicked off if the
		 * channel (read connection) doesn't exist or if the channel has died.
		 */
		try {
			if (channel == null) {
			...
			if ((channel == null)
				...
				connectionFactory.setRequestedHeartbeat(Integer
						.parseInt(SystemProperties.get("ss.conn_heartbeat")));
				String host = resourceQURI.getHost();
				connectionFactory.setHost(host);
				String virtualHost = uriDecode(path.substring(1,
						path.indexOf('/', 1)));
				...
				/*
				 * And create a consumer using the first queue. This consumer
				 * allows subsequent queue listeners to be added and removed as
				...
				for (int retries = 1; retries <= CONNECT_RETRIES; retries++) {
					if (connectSuccess == false) {
						...
						try {
							...
							// we're catching this and ignoring it during the
							// retries
						}
				...
				/*
				 * A map to used to keep a tally of which queue listeners (cTag)
				 * have been created and to disconnect later on when all we get
				 * is the resource Id. Disconnection can only happen via a cTag.
				 */
				resourceChannMap.put(resourceId, ctag);

				/*
				 * A map used by RabbitMqConsumer to tie a cTag (which is all it
				 * gets from RabbitMq) to identify which resource an echo
				 * response came in for.
				 */
				CtagResourceMap.addCtag(ctag, resourceId);
				EchoResourceMap.getEchoMap().addResource(resourceId);
				logger.info("Initial basic consumer " + ctag
						+ " added for queue " + queue + "for resource "
						+ resourceId);
			}

			/*
			 * This section is the loop which uses the connection opened above
			 * and adds additional consumers as they are requested. The two maps
			 * are also updated here. This loop constantly monitors
			 * resourceSessionList for any new pending additions to the number
			 * of active queue listeners. Could have used an observer to add
			 * more listeners, but that happens very infrequently compared to
			 * the lifetime of the running program, this is simpler for the
			 * return you get.
			 */
				...
				while (resourceSessionList.isEmpty() == false) {
					...
					try {
						...
						if (resourceChannMap.containsKey(resourceId) == false) {
							...
							resourceChannMap.put(resourceId, ctag);
							CtagResourceMap.addCtag(ctag, resourceId);
							EchoResourceMap.getEchoMap()
									.addResource(resourceId);
							logger.info("Additional basic consumer " + ctag
						...
						logger.error("Queue name corrupted: "
								+ session.getAmqpDest());
					}
				}
				Thread.sleep(1000);
			}
			// for java 1.7 this syntax is preferable: catch(URISyntaxException
			// | UnsupportedEncodingException ex)
		} catch (AlreadyClosedException ex) {
			logger.error("The amqp channel is closed: " + ex.getMessage());
		} catch (URISyntaxException ex) {
			logger.error("URI: " + session.getAmqpDest() + " for session: "
	...
	protected static void removeMapping(String cTag) {
		EchoResourceMap.getEchoMap().removeResource(
				CtagResourceMap.getResource(cTag));
	 ...
	 * fixture/resource. The loop in run() above will pick this up and crete the
	 * listner there.
	 */
