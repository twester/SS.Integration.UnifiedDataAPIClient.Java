...
public class ResourceImpl implements Resource {
	private static Logger logger = Logger.getLogger(ResourceImpl.class
			.getName());
	private static HttpServices httpSvcs = new HttpServices();
	private static ExecutorService actionExecuter = Executors
	...
	/*
	 * Constructor initializes and resets internal state in case it is
	 * re-initialized by the client code.
	 */
	...
	private void connect() {
		if (connected == false) {
			...
			amqpRequest = httpSvcs.processRequest(availableResources,
					"http://api.sportingsolutions.com/rels/stream/amqp",
					restItem.getName());
			amqpDest = amqpRequest.getServiceRestItems().get(0).getLinks()
					.get(0).getHref();
			logger.info("Starting new streaming service, name: " + getName()
					+ " queue: " + amqpDest + " fixture ID: " + getId());

			/*
			 * We need these three services to be running for the SDK to behave
			 * as expected. And as they need the MQ details (which are only
			 * available at this point) we are starting them here. We could
			 * retrieve and set this up earlier, but it that would mean querying
			 * teh client resources twice on start up.
			 */
			...
			ServiceThreadExecutor.executeTask(MQListener
					.getMQListener(amqpDest));
			EchoSender echoSender = EchoSender.getEchoSender(amqpDest,
					availableResources);
			ServiceThreadExecutor.executeTask(echoSender);

			// MQListener.setResources does not allow duplicates so fall
			// throughs from the above false will be ignored
			MQListener.setResources(new ResourceSession(amqpDest, getId()));
